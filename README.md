...
| email notifications | In case that some cron jobs stuck, notifications will be sent over email. In extreme cases the cron jobs will be stopped. |
...
| `php` version | You can choose the `php` version to install |
| mautic 4 version | You can choose the `mautic 4` version to install (the script is not suitable for `mautic 5`) |
| `adminer` | To access detailed informations in the database, [adminer evo](https://github.com/adminerevo/adminerevo) will be installed |
| commands | Utility that allows you to run different commands without `ssh` |
...
| `mariadb` | `mariadb` is used for database |
...
| icons | You can use usefull icons of social platforms inside your emails/landing pages |
| ro language | Romanian language pack (partially translated) (with some improvements from the oficial version) |
| `local.php` | Mautic will be already initialized |
| owner of contacts | Assign new contacts to first mautic user by default |
| `MauticAdvancedTemplatesBundle` | Plugin to enable `TWIG code` inside emails, landing pages and dynamic content |
| `MauticPostalServerBundle` | To be able to send emails trough `Postal API` (there is an issue about the certification of the email) |
...
1. Connect trough `ssh` to the server (I use Termius)
1. Run as the root user:
```sh
bash <(wget -qO- https://raw.githubusercontent.com/IonutOjicaDE/mautic4-installer/main/mautic-install.sh)
```

* the installer will automatically download and open the `config file` in the `nano` text editor if it is not found in the current directory
...
If it is not found, then a template configuration file will be downloaded from [github](https://raw.githubusercontent.com/IonutOjicaDE/mautic4-installer/main/mautic-install.conf) and will be opened in the `nano` text editor (you can find some shortcuts for `nano` below).
...
If the Soft Limit is exceeded, then an email will be sent out to the admin with the information.
...
Some actions are executed only if there is a need for, like `emails:send` or `import`.
...
Actually is self explicit: it clears the Mautic cache. This jobs is executed over night.
...
Actually is self explicit: this job is executed from the crontab of the root user each night.
...
# Adminer
Adminer will be accessible at https://mautic-subdomain/database/database.php . There you should enter the root user and the password for the root user in MySQL database you received as `MYSQL_ROOT_PASSWORD`.

Some plugins are already installed: edit-textarea, file-upload, foreign-system, tables-filter, tinymce.
...
With this installation you will have a fully functional live webinar automation, meaning all emails, campaigns, forms, segments are there and connected. You have to personalize the emails, insert the form to your landing page, set the date of the next webinar and let the interested contacts register.

The name of these elements (emails, campaigns, forms, segments) is all in romanian language.

Also all contact fields are renamed in romanian language.

# To do
1. The backup files are only on the server - this is not good: if the server will be unavailable, the backups will not be accessible anymore. Upload to external cloud is needed, perhaps each 7-th day. [Hetzner](https://ionutojica.com/hetzner) (or DigitalCloud) can make also backups, but these are also on their servers - it is better to have an external backup also.
...
1. Install only one Adminer. Now each Mautic installation has also an Adminer installed, although one is enough.
1. Download whole repository as a zip and execute the scripts from there. Now we download each script at a time and execute it.
