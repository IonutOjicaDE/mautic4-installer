...
show_info 🛈 'Install netcat-openbsd (to check the authentification to the email server trough nc)...'
DEBIAN_FRONTEND=noninteractive apt-get -yq install netcat-openbsd
show_info 🛈 'Install cron (needed for scheduled actions for Mautic / cronjobs)...'
DEBIAN_FRONTEND=noninteractive apt-get -yq install cron
show_info 🛈 'Install unzip...'
DEBIAN_FRONTEND=noninteractive apt-get -yq install unzip

show_info 🛈 'Install the needed packages to add new sources of packages in APT...'
...
while true; do
  ...
  if [[ -r "${PWD}${FILE_CONF}" ]]; then
    ...
    if apt-cache show php${PHP_VERSION} > /dev/null 2>&1; then
      show_info 🛈 "Install php${PHP_VERSION}..."
